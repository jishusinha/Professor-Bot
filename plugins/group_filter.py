...
async def fil_mod(client, message): 
      ...
      if args in mode_on:
          ...
          await m.edit("**Aᴜᴛᴏꜰɪʟᴛᴇʀ Eɴᴀʙʟᴇᴅ**")
      
      elif args in mode_of:
          FILTER_MODE[str(message.chat.id)] = "False"
          await m.edit("**Aᴜᴛᴏꜰɪʟᴛᴇʀ Dɪꜱᴀʙʟᴇᴅ**")
      else:
          await m.edit("ᴜꜱᴇ :- `/autofilter on` Oʀ `/autofilter off`")
...
async def g_fil_mod(client, message): 
      ...
      if args in mode_on:
          ...
          await m.edit("**Gʟᴏʙᴀʟ Fɪʟᴛᴇʀ Eɴᴀʙʟᴇᴅ**")
      
      elif args in mode_of:
          G_MODE[str(message.chat.id)] = "False"
          await m.edit("**Gʟᴏʙᴀʟ Fɪʟᴛᴇʀ Dɪꜱᴀʙʟᴇᴅ**")
      else:
          await m.edit("ᴜꜱᴇ :- `/g_filter on` Oʀ `/g_filter off`")
...
async def next_page(bot, query):
    ...
    btn.insert(0, [InlineKeyboardButton("🔗 Hᴏᴡ Tᴏ Dᴏᴡɴʟᴏᴀᴅ 🔗", "howdl")])
    if 0 < offset <= 10:
        off_set = 0
    elif offset == 0:
        off_set = None
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⬅️ Bᴀᴄᴋ", callback_data=f"next_{req}_{key}_{off_set}"),
             InlineKeyboardButton(f"❄️ Pᴀɢᴇꜱ {math.ceil(int(offset) / 10) + 1} / {math.ceil(total / 10)}", callback_data="pages")]
        )
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        req = message.from_user.id if message.from_user else 0
        btn.append(
            [InlineKeyboardButton(text=f"❄️ Pᴀɢᴇꜱ 1/{math.ceil(int(total_results) / 6)}", callback_data="pages"),
             InlineKeyboardButton(text="➡️ Nᴇxᴛ", callback_data=f"next_{req}_{key}_{offset}")]
        )
    else:
        btn.append(
            [InlineKeyboardButton(text="❄️ Pᴀɢᴇꜱ 1/1", callback_data="pages")]
        )
